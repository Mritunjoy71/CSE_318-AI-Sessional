...
int main(){
	...
	if (board[6] > board[13]){
		...
		cout << "Computer1 wins, as expected." << endl;
	}
	else {
...
int evalFunction(int board[], int minOrMax){ 
	...
	float humanside = (board[0] + board[1] + board[2] + board[3] + board[4] + board[5])*0.45; 
	float humanmancala = (board[6]) * 3.45; 
	float human = humanside + humanmancala;
	float compside = (board[7] + board[8] + board[9] + board[10] + board[11] + board[12])*0.45;
	float compmancala = (board[13]) * 3.45;
	...
	checkempty = takeMove(board2, i, minOrMax); // landing in an empty bin
		if(checkempty){ 
			...
			valueofboard = valueofboard + ((acrosspieces*0.5));//not interested so negative
		}
	}
