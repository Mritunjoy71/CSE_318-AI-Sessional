...
int evalFunction(int board[], int minOrMax){ 
	...
	float comp2mancala = (board[6]); 
	
	float comp1mancala = (board[13]);
	float difference_board =(comp1mancala-comp2mancala)*w2;
	...
	checkempty = takeMove(board2, i, minOrMax);
		if(checkempty){
			...
			valueofboard = valueofboard + ((acrosspieces*0.5)*w1);
		}
	}
	for (int i = 7; i <= 12; i++) { 
	checkempty = takeMove(board2, i, minOrMax); 
		if(checkempty){
			int acrosslocation = findacross(i);
			int acrosspieces = board2[acrosslocation];
			valueofboard = valueofboard + ((acrosspieces*0.5)*w2); 
		}
	}
	*/
	return valueofboard; 
